...
import { formatArea, formatEquipmentList, formatHouseTypeName, formatLabelList, formatOpenQueue, formatPriceRange, formatRoomType, formatRoomTypeCode } from '@/utils/format'
...
// 获取房型第一张图片（用于卡片显示）
function getHouseTypeImage(house: HouseType): string | undefined {
  if (!house.roomPicUrl)
    return undefined
...
// 获取房型所有图片
function getHouseTypeImages(house: HouseType): string[] {
  if (!house.roomPicUrl)
    return []
...
// 打开VR链接
function openVrUrl(vrUrl: string) {
  if (vrUrl) {
...
// 打开房型图片查看器
function openHouseTypeViewer(house: HouseType) {
    ...
    <div class="container-app py-4 md:py-6">
      ...
      <div v-else-if="property" class="space-y-4 md:space-y-6">
            ...
            <div class="grid grid-cols-2 gap-3">
              <div class="bg-gray-50 rounded-lg p-3">
                ...
                <p class="text-sm font-semibold text-red-600">
                  {{ formatPriceRange(property.minRent, property.maxRent)
                  }}/月
                </p>
              </div>
              <div class="bg-gray-50 rounded-lg p-3">
        ...
        <div v-if="property.roomTypeDetails && property.roomTypeDetails.length > 0" class="space-y-4">
          ...
          <div
            v-for="(detail, index) in property.roomTypeDetails" :key="index"
          ...
          >
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <div
                v-for="(house, hIndex) in detail.houseTypeList" :key="hIndex"
                class="border border-gray-200 rounded-lg overflow-hidden hover:shadow-md transition-shadow"
              >
                <!-- 房型图片 -->
                <div class="bg-gray-100 relative">
                  <img
                    v-if="getHouseTypeImage(house)" :src="getHouseTypeImage(house)" :alt="house.houseTypeName"
                ...
                <div class="p-3">
                  ...
                  <!-- 标签 -->
                  <div
                    v-if="house.roomLabel && formatLabelList(house.roomLabel).length > 0"
                    class="flex flex-wrap gap-1 mb-2"
                  >
                    <span
                      v-for="(label, lIndex) in formatLabelList(house.roomLabel)" :key="lIndex"
                  ...
                  <!-- 设备 (全部显示) -->
                  <div
                    v-if="house.roomEquipment && formatEquipmentList(house.roomEquipment).length > 0"
                    class="flex flex-wrap gap-1"
                  >
                    <span
                      v-for="(eq, eIndex) in formatEquipmentList(house.roomEquipment)" :key="eIndex"
        ...
        <div v-if="property.latitude && property.longitude" class="bg-white rounded-xl shadow-md p-4 md:p-6">
          ...
          <AmapNearby
            v-if="showNearby" :latitude="property.latitude" :longitude="property.longitude"
