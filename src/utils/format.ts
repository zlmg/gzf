...
export const formatRoomTypeCode = (code: string): string => {
  if (!code) return '-'
  ...
  let result = ''
  if (room) result += `${room}室`
  if (hall) result += `${hall}厅`
  if (bath) result += `${bath}卫`
...
// 标签列表格式化
export const formatLabelList = (label: string): string[] => {
  if (!label) return []
...
export const formatHouseTypeName = (name: string): string => {
  if (!name) return '-'
